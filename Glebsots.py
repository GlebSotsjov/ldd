...
if name.isalpha():
    ...
    print("Tähtede arv nimes:", len(name))
    vowels = sum(1 for letter in name.lower() if letter in 'aeiouаеёиоуыэюя')
    consonants = len(name) - vowels
    print("Vokaaltähtede arv:", vowels)
    print("Konsonantide arv:", consonants)
    print("Nime tähed tähestikulises järjekorras:", ''.join(sorted(set(name.lower()))))
else:
...
if len(set(numbrid)) < 2:
    print("Loendis peab olema vähemalt kaks erinevat arvu.")
else:
    min_arv = min(numbrid)
    max_arv = max(numbrid)
    
    min_indeks = numbrid.index(min_arv)
    max_indeks = numbrid.index(max_arv)
    
    numbrid[min_indeks], numbrid[max_indeks] = numbrid[max_indeks], numbrid[min_indeks]
    
    print("Algseisund:", numbrid)
    print("Miinimumarv on asendatud maksimumarvuga ja vastupidi:", numbrid)

#13

import random

sonad = ["kass", "koer", "hiir", "hunt", "jänes"]

valitud_sona = random.choice(sonad)
alljooned = ["_" for _ in valitud_sona]
arvatud_tahed = []
katsete_arv = 0

while "_" in alljooned:
    print(" ".join(alljooned))
    taht = input("Sisesta täht: ").lower()
    if taht in valitud_sona:
        for i in range(len(valitud_sona)):
            if valitud_sona[i] == taht:
                alljooned[i] = taht
    else:
        arvatud_tahed.append(taht)
        print("Vale tähted:", arvatud_tahed)
    katsete_arv += 1

print("Õige sõna:", valitud_sona)
print("Katsete arv:", katsete_arv)

#16
 
import random

responses = ["Jah, kindlasti!", "Jah!", "Võib-olla!", "Ei!"]
...
if küsimus == "jah":
    vastus = random.choice(responses)
...
words = ["kass", "koer", "hiir", "hunt", "jänes", "krokodill", "kodu", "kivi", "kassipoeg", "kaktus"]

otsingusona = input("Sisestage otsingusõna: ")

for sõna in words:
    if otsingusona in sõna:
        print(sõna)
